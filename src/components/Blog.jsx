  ...
  return (
    <section id="blog" className="py-16 sm:py-24 bg-slate-100">
      <div className="max-w-6xl mx-auto px-6">
        ...
        {/* Blog cards — uniform 2-col grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {blogs.map((blog, i) => (
            <Link
              key={blog.id}
              to={`/blog/${blog.slug}`}
              className="group border border-gray-200 bg-white rounded-2xl overflow-hidden hover:border-blue-200 hover:shadow-md transition-all duration-200"
            >
              <div className={`bg-gradient-to-br ${thumbGradients[i % thumbGradients.length]} h-32 flex items-end p-5`}>
                <span className="text-white/70 text-xs font-medium">{blog.date} · {blog.readTime}</span>
              </div>
              <div className="p-5">
